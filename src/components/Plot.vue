    ...
    <div class="pageoverlay">
        <div class="overlaybox">
            ...
            <div class="overlaycontent">
                ...
                <line-chart ref="LineChart" v-else :chartdata="chartData" :options="chartOptions" :styles="chartStyle"/>
...
export default {
    ...
    methods:{
        loadGraph(){
            HTTP.get(this.apiURL+'/selectors/'+this.id_selector+'/results/'+this.id_result+'/tests/'+this.id_test+'/float').then((resp)=>{
                this.test = resp.data.test
                this.chartData.datasets=[]
                let temp = {}
                temp = DataLoader.getcolor('value')
                var ylabel = this.test.units
                ...
                DataLoader.loadGraph(this.id_selector, this.id_result, this.id_test, this.test.type).then((resp)=>{
                    ...
                    DataLoader.loadLimits(this.id_selector,this.id_result,this.id_test,this.test.type).then((limit)=>{
                        if (limit){
                            for (let j=0;j<limit.length;j++){
                                var category, label
                                if(j==0){
                                    category='limitdanger';
                                    label='min'
                                } else if(j==1){
                                    category='limitwarning';
                                    label='low'
                                } else if(j==2){
                                    category='limitwarning';
                                    label='high'                            
                                ...
                                var limitset = DataLoader.getcolor(category)
                                limitset.data=[{
